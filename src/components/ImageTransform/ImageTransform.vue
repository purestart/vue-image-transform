...
export default {
  ...
  created() {
    ...
    image.onload = function() {
      ...
      that.initPoints[1][0] =
        (that.width - that.imageWidth) / 2 + that.imageWidth;
      that.initPoints[1][1] = (that.height - that.imageHeight) / 2;

      that.initPoints[2][0] =
        (that.width - that.imageWidth) / 2 + that.imageWidth;
      that.initPoints[2][1] =
        (that.height - that.imageHeight) / 2 + that.imageHeight;
  ...
  methods: {
    initCanvas() {
      ...
      $(".matrix-img").css(
        "background-size",
        this.imageWidth + "px " + this.imageHeight + "px"
      );
      ...
      pts.on(this.eventList.TOUCH_START, function(e) {
        ...
        that.targetPoint = target.hasClass("tl")
          ? transform.topLeft
          : target.hasClass("tr")
            ? transform.topRight
            : target.hasClass("bl")
    ...
    getDecodeImage(cb, points) {
      ...
      img.addEventListener(
        ...
        function() {
          ...
          //获取基点
          let basex =
            basePoints[0][0] > basePoints[3][0]
              ? basePoints[3][0]
              : basePoints[0][0];
          let basey =
            basePoints[0][1] > basePoints[1][1]
              ? basePoints[1][1]
              : basePoints[0][1];
